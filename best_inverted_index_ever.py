    ...
    def read(self, locs, n_bytes,name):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                gs_name = f"postings_gcp_{name}/{f_name}"
                blob = self.bucket.blob(gs_name)
...
class InvertedIndex:  
    ...
    def read_posting_list(self,word,bucket_name,name):
        """ A generator that reads one posting list from disk and yields 
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
        """
        if(self.df.get(word) is None):
            return []
        with closing(MultiFileReader(bucket_name)) as reader:
            locs = self.posting_locs[word]
            b = reader.read(locs, self.df[word] * TUPLE_SIZE,name)
            posting_list = []
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl: 
                ...
                b = b''.join([(doc_id << 16 | (tf & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
                              for doc_id, tf in pl])
                # write to file(s)
                locs = writer.write(b,name)
                # save file locations to index
                posting_locs[w].extend(locs)
            writer._f.close()
            writer.upload_to_gcp(name) 
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name,name):
        ...
        blob_posting_locs = bucket.blob(f"postings_gcp_{name}/{bucket_id}_posting_locs.pickle")
